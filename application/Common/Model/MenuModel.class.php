...
class MenuModel extends CommonModel {
    ...
    public function admin_menu($parentid, $with_self = false) {
        ...
        //权限检查
        if (sp_get_current_admin_id() == 1||$_SESSION['user']['isadmin']==2) {
            //如果是超级管理员 直接通过          
            return $result;
        }else{
            $array = array(); 
            ...
            $authid1=$org->where('id='.$loginuser['oid'])->getField('authority');          
            if(empty($authid1)) //如果从组织表中查询到的authorityid未空，只代表当前用户没有权限访问
                return $array;           
            $authid= explode(',', $authid1); 
            foreach($result as $key=>$val){
                if(in_array($val['id'],$authid)){
                    $array[]=$val;
                }
            }
        }          
    ...
    public function menu_json() {
        $data = $this->get_tree(0);     // var_dump($data);die;
        return $data;
    }
    //取得树形结构的菜单
    public function get_tree($myid, $parent = "", $Level = 1) {          
        ...
        if (is_array($data)) { 
            ...
            foreach ($data as $a) {   
                ...
                $menuurl="{$name}/{$model}/{$action}{$params}";
                //$lang="$name.'_'.$model.'_'.$action";
                if($a['isreport']){//如果是访问的是报表，要跳转的url为$url的值                  
                    $realurl=$url.$params;
                    $rid=$id.'/'.$realurl;
                 
                }else{//当前要访问的是菜单，则要链接的url为三个值拼接起来的值
                    $realurl=U($menuurl);
                    $rid=$id.$realurl;
                }          
                $array = array(
                ...
                $ret[$id.$realurl] = $array; 
                $child = $this->get_tree($a['id'], $id, $Level);
                //由于后台管理界面只支持三层，超出的不层级的不显示
                if ($child && $Level <= 3) {
                    //$ret[$id . $realurl]['items'] = $child;
                     $ret[$rid ]['items'] = $child;
                }                        
            }
            return $ret;
    ...
    public function menu_cache($data = null) { 
        if (empty($data)) {
            $loginuser=$_SESSION['user'];            
            $loginoid=$loginuser['oid'];
            if($loginuser['isadmin']==2){  //如果当前登录的是超级管理员，则显示所有的权限
               $data = $this->where('status=1')->select();
            }else{//如果当前登录的是某个单位的管理员，则显示其当前父类的所拥有的权限
                $org=M('Orginization');
                $currentorg=$org->where('id='.$loginoid)->find();           
                $authid=$org->where('id='.$currentorg['parentid'])->getField('authority'); 
                //$authid=$org->where('id='.$currentorg['id'])->getField('authority'); 
                if($authid){
                    $where['id']=array('in', explode(',', $authid));
