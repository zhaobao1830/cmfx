...
class AdminbaseController extends AppframeController {
	...
	function _initialize(){
	    ...
	    $session_admin_id=$_SESSION['user']['id'];
		...
		$this->assign("admin",$user);
			...
			if(IS_AJAX){
				//$this->error("您还没有登录！",U("admin/public/login"));
                                $this->error("您还没有登录！",U("User/Caslogin/caslogin"));
			}else{
				//header("Location:".U("admin/public/login"));
                                header("Location:".U("User/Caslogin/caslogin"));
				exit();
			}
	...
	public function initMenu() {
        ...
        //查询一个人所属公司级的departmenttid
     public function getDepartmentid($pathid,$departmentid){     
        $departarr= explode(',', $departmentid);  
        $level=0;
        foreach ($departarr as $key=>$val){                  
            if(mb_substr($val, -2,2,'utf-8')=='公司'){
               $level=$key;             
               break;
            }
        }
        $len= strlen($pathid);
        $parpathid=substr($pathid,0,$len-$key*3);     
        $org=M('Orginization');
        $departid=$org->where('level='.$parpathid)->find();      
        return $departid;
    }
