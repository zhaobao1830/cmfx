...
class MenuController extends AdminbaseController {
    ...
    //不返回组织节点下的用户列表
    public function getOrginization(){  
           $org=M('Orginization');          	              
           $return=$org->field('id,parentid,org_name,authority')->select();                                                        
           return $return; 
    ...
    public function add() {
        ...
        $this->assign("orglist", json_encode($orglist));
    	$this->display();
    }
    				...
    				$oid=$_POST['oid'];//逗号分隔
                                ...
                                if($data['isreport']==1){
                                    ...
                                    if(empty($data['action'])){
                                         $this->error("action 不能为空");
                                    }
                                    $mwhere=array("name"=>$name); 
                                }                              
                                if ($this->menu_model->add($data)!==false) {
                                    $org=M('Orginization');
                                    $menuid=$this->menu_model->getLastInsID();
                                    if($oid){                                              
                                        //$sql="update ".C(DB_PREFIX)."orginization set authority=concat_ws(',','{$menuid}',authority) where id in ($oid)";                                  
                                        $res=$org->execute("update ".C(DB_PREFIX)."orginization set authority=concat_ws(',','{$menuid}',authority) where id in ($oid)");                                       
                                    }
