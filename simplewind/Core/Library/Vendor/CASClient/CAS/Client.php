...
class CAS_Client
{
	...
	public function getServerClientId()
	{
		return $this->_server['client_id'];
	}
	public function getServerClientSecret()
	{
		return $this->_server['client_secret'];
	}
	/**
	...
	public function getServerLoginURL($gateway=false,$renew=false)
	{
		...
		if ( empty($this->_server['login_url']) ) {
			...
			$this->_server['login_url'] .= '&client_id=';
			$this->_server['login_url'] .= $this->getServerClientId();
			$this->_server['login_url'] .= '&secret=';
			$this->_server['login_url'] .= $this->getServerClientSecret();
		}
	...
	$changeSessionID = true
		...
		$this->_server['client_id']= $clientid;
		$this->_server['client_secret']= $cllientsecret;
		// check URI
	...
	public function logout($params)
	{
		...
		if (isset($params['client_id'])) {
			//$cas_url = $cas_url . $paramSeparator . "service=" . urlencode($params['service']);
			$cas_url = $cas_url."&client_id=".urlencode($params['client_id']);
		}
                unset($_SESSION);
		session_unset();
		session_destroy();
	...
	public function validateCAS20(&$validate_url,&$text_response,&$tree_response)
	{
		...
		if ( !($dom->loadXML($text_response))) {
			...
			if ( $success_elements->item(0)->getElementsByTagName("user")->length == 0) {
				...
				$proxyList = array();
				$strtmp=explode("~",$success_elements->item(0)->getElementsByTagName("attributes")->item(0)->getElementsByTagName("access_token")->item(0)->nodeValue);
                $_SESSION['CAS']['ast']=$strtmp[0];
                $_SESSION['CAS']['rft']=$strtmp[1];
	...
	public function getURL()
	{
		...
		//file_put_contents("aa1.txt",$this->_url."---111");
		return $this->_url;
	}
